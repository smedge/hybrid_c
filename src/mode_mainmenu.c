...
static ConfirmDialog newGameDialog;
...
static void new_game_clicked(void)
{
	if (Savepoint_has_save_file()) {
		ConfirmDialog_open(&newGameDialog,
			"This will erase previously saved data.");
	} else {
	...
	const unsigned int ticks)
{
	...
	newButton.position.y = menu_top + 110 * s;
	loadButton.position.y = menu_top + 135 * s;
	exitButton.position.y = menu_top + 160 * s;

	if (newGameDialog.open) {
		ConfirmDialog_update(&newGameDialog, input);
...
void Mode_Mainmenu_render(void)
{
	...
	ConfirmDialog_render(&newGameDialog);
